...
interface HarassmentStatus {
  name: string;
  status: string;
}

const HarassmentStatusComponent = () => {
  const [open, setOpen] = useState(false);
  const [editingStatus, setEditingStatus] = useState<HarassmentStatus | null>(null);
  const [statuses, setStatuses] = useState<HarassmentStatus[]>([]);
  ...
  useEffect(() => {
    const savedStatuses = JSON.parse(localStorage.getItem('harassmentStatuses') || '[]');
  ...
  const handleDelete = (name: string) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        ...
        localStorage.setItem('harassmentStatuses', JSON.stringify(updatedStatuses));
  ...
  // Handle edit status
  const handleEdit = (status: HarassmentStatus) => {
...
export default HarassmentStatusComponent;
