...
const LeadStatus = () => {
  ...
  const [statuses, setStatuses] = useState<LeadStatusType[]>([]);
  ...
  // Handle edit status
  const handleEdit = (status: LeadStatusType) => {
  ...
  useEffect(() => {
    const savedStatuses = JSON.parse(localStorage.getItem('leadStatuses') || '[]');
