...
const AccountStatus = () => {
  ...
  const [editingStatus, setEditingStatus] = useState<AccountStatusType | null>(null);
  const [statuses, setStatuses] = useState<AccountStatusType[]>([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      try {
        ...
        localStorage.setItem('accountStatuses', JSON.stringify(updatedStatuses));
  ...
  // Handle edit status
  const handleEdit = (status: AccountStatusType) => {
  ...
  useEffect(() => {
    const savedStatuses = JSON.parse(localStorage.getItem('accountStatuses') || '[]');
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth PaperProps={{ style: { borderRadius: '16px', padding: '16px' } }}>
        ...
        >
          {editingStatus ? 'Edit Account Status' : 'Add Account Status'}
        </DialogTitle>
