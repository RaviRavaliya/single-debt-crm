...
const SourceStatus = () => {
  ...
  const [editingStatus, setEditingStatus] = useState<SourceStatusType | null>(null);
  const [statuses, setStatuses] = useState<SourceStatusType[]>([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      try {
        ...
        localStorage.setItem('sourceStatuses', JSON.stringify(updatedStatuses));
  ...
  // Handle edit status
  const handleEdit = (status: SourceStatusType) => {
  ...
  useEffect(() => {
    const savedStatuses = JSON.parse(localStorage.getItem('sourceStatuses') || '[]');
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth PaperProps={{ style: { borderRadius: '16px', padding: '16px' } }}>
        ...
        >
          {editingStatus ? 'Edit Source Status' : 'Add Source Status'}
        </DialogTitle>
