...
const EducationStatus = () => {
  ...
  const [editingStatus, setEditingStatus] = useState<EducationStatusType | null>(null);
  const [statuses, setStatuses] = useState<EducationStatusType[]>([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      try {
        ...
        localStorage.setItem('educationStatuses', JSON.stringify(updatedStatuses));
  ...
  // Handle edit status
  const handleEdit = (status: EducationStatusType) => {
  ...
  useEffect(() => {
    const savedStatuses = JSON.parse(localStorage.getItem('educationStatuses') || '[]');
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth PaperProps={{ style: { borderRadius: '16px', padding: '16px' } }}>
        ...
        >
          {editingStatus ? 'Edit Education Status' : 'Add Education Status'}
        </DialogTitle>
