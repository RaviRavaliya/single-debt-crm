...
interface AccommodationStatusType {
  name: string;
  status: string;
}
...
const AccommodationStatus = () => {
  ...
  const [editingStatus, setEditingStatus] = useState<AccommodationStatusType | null>(null);
  const [statuses, setStatuses] = useState<AccommodationStatusType[]>([]);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      try {
        ...
        localStorage.setItem('accommodationStatuses', JSON.stringify(updatedStatuses));
  ...
  const handleDelete = (name: string) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        const updatedStatuses = statuses.filter((status) => status.name !== name);
        localStorage.setItem('accommodationStatuses', JSON.stringify(updatedStatuses));
        setStatuses(updatedStatuses);
  ...
  // Handle edit status
  const handleEdit = (status: AccommodationStatusType) => {
    setEditingStatus(status);
    formik.setValues(status);
    handleOpen();
  };

  // Load statuses from localStorage when the component mounts
  useEffect(() => {
    const savedStatuses = JSON.parse(localStorage.getItem('accommodationStatuses') || '[]');
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Dialog open={open} onClose={handleClose} fullWidth PaperProps={{ style: { borderRadius: '16px', padding: '16px' } }}>
        ...
        >
          {editingStatus ? 'Edit Accommodation Status' : 'Add Accommodation Status'}
        </DialogTitle>
